...
static const u_char *md5_body(md5_t ctx, const u_char *data,
    size_t size);
static void hex_from_raw( unsigned char *in, int inlen, char *out);
...
md5_update(md5_t ctx, const void *data, size_t size)
{
	...
	memset(ctx,0,sizeof(struct md5_st));
	ctx->a = 0x67452301;
    ctx->b = 0xefcdab89;
    ctx->c = 0x98badcfe;
    ctx->d = 0x10325476;
...
md5_final( md5_t ctx)
{
    ...
    tmp[0] = (u_char) ctx->a;
    tmp[1] = (u_char) (ctx->a >> 8);
    tmp[2] = (u_char) (ctx->a >> 16);
    tmp[3] = (u_char) (ctx->a >> 24);
    tmp[4] = (u_char) ctx->b;
    tmp[5] = (u_char) (ctx->b >> 8);
    tmp[6] = (u_char) (ctx->b >> 16);
    tmp[7] = (u_char) (ctx->b >> 24);
    tmp[8] = (u_char) ctx->c;
    tmp[9] = (u_char) (ctx->c >> 8);
    tmp[10] = (u_char) (ctx->c >> 16);
    tmp[11] = (u_char) (ctx->c >> 24);
    tmp[12] = (u_char) ctx->d;
    tmp[13] = (u_char) (ctx->d >> 8);
    tmp[14] = (u_char) (ctx->d >> 16);
    tmp[15] = (u_char) (ctx->d >> 24);
	
	hex_from_raw(tmp,16,ctx->result);
}
