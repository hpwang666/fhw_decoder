...
rtspClient_t init_rtsp_clients(pooList_t list,char *ip,int port,char *user,char *passwd,char *url)
{
	int ret;
...
static int send_setup(rtspClient_t rc)
{
	...
	if(rc->sess->control){
		if(rc->sess->contentBase){
			if(strnstr(rc->sess->control->data,rc->sess->contentBase->data,rc->sess->control->len))
			{
...
static int  search_rtsp_head(buf_t buf)
{
	...
	u_char *p = strnstr(data, "RTSP", size);
	if (p) return (p-data); 
...
static int do_response(rtspClient_t rc,u_char* data, size_t len)
{
	...
	char spsB64[128];
	int decodeLen=0;
	head =(char *)strnstr(data,"RTSP/1.0 ",len);
	if(!head) return RTSPERR;
	rep = atoi(head+9);
	
	if(rep == RTSP401){
		head = (char *)strnstr(data, "nonce",len);
		if(!head) 
		{
			head = (char *)strnstr(data, "Basic",len);
			if(!head) return RTSPERR;
		}
		else{
			head+=7;
			...
			head =(char *) strnstr(data,"realm",len);
			if(!head) return RTSPERR;
			head+=7; 
			tail = strchr(head,'"');
			if(!tail) return RTSPERR;
	...
	head = (char *)strnstr((u_char *)data,"control:",len);
	if(head ){
		head+=8;
		head = (char *)strnstr((u_char *)head,"control:",len-((u_char*)head-data));
		if(head){
			tail=(char *)strnstr((u_char *)head,"\r\n",len-((u_char*)head-data));
			if(tail){
				head+=8;
	...
	head = (char *)strnstr((u_char *)data,"Content-Base:",len);
	if(head ){
		tail=(char *)strnstr((u_char *)head,"\r\n",len-((u_char*)head-data));
		if(tail){
			head+=14;
	...
	head = (char *)strnstr((u_char*)data,"sprop-parameter-sets",len);
	if(head){
		head +=strlen("sprop-parameter-sets")+1;
		tail = (char*)strnstr((u_char *)head,",",64);
		if(tail){
	...
	head =(char *)strnstr(data,"Session:",len);
...
static void generate_auth(rtspClient_t rc, char* cmd)
{
	...
	if(rc->sess->nonce){
		...
		b64_encode(base64In->data, base64Out->data,base64In->len);
