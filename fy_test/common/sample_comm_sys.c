...
******************************************************************************/
FY_U32 SAMPLE_COMM_SYS_CalcHistVbBlkSize(VIDEO_NORM_E enNorm, PIC_SIZE_E enPicSize, SIZE_S *pstHistBlkSize, FY_U32 u32AlignWidth)
{
    FY_S32 s32Ret;
    SIZE_S stPicSize;
...
FY_U32 SAMPLE_COMM_SYS_CalcPicVbBlkSize(VIDEO_NORM_E enNorm, PIC_SIZE_E enPicSize, PIXEL_FORMAT_E enPixFmt, FY_U32 u32AlignWidth,COMPRESS_MODE_E enCompFmt)
{
    ...
    else if (COMPRESS_MODE_SLICE == enCompFmt || COMPRESS_MODE_FBC == enCompFmt)
    {
        u32HeaderSize = 0x40;
    }
...
FY_S32 SAMPLE_COMM_SYS_Init(VB_CONF_S *pstVbConf)
{
    ...
    if (FY_SUCCESS != s32Ret)
    {
        SAMPLE_PRT("FY_MPI_VB_SetConf failed!\n");
        return FY_FAILURE;
    }
    ...
    if (FY_SUCCESS != s32Ret)
    {
        SAMPLE_PRT("FY_MPI_VB_Init failed!\n");
        return FY_FAILURE;
    }
    ...
    if (FY_SUCCESS != s32Ret)
    {
        SAMPLE_PRT("FY_MPI_SYS_SetConf failed\n");
        return FY_FAILURE;
    }
    ...
    if (FY_SUCCESS != s32Ret)
    {
        SAMPLE_PRT("FY_MPI_SYS_Init failed!\n");
        return FY_FAILURE;
    }
