...
inline FY_U16 OSD_MAKECOLOR_U16(FY_U8 r, FY_U8 g, FY_U8 b, OSD_COMP_INFO compinfo)
{
    FY_U8 r1, g1, b1;
    FY_U16 pixel = 0;
    FY_U32 tmp = 15;
...
FY_S32 GetBmpInfo(const char *filename,     OSD_BITMAPFILEHEADER  *pBmpFileHeader
...
int LoadBMP(const char *filename, OSD_LOGO_T *pVideoLogo)
{
    ...
    pVideoLogo->width = (FY_U16)bmpInfo.bmiHeader.biWidth;
    pVideoLogo->height = (FY_U16)((bmpInfo.bmiHeader.biHeight>0)?bmpInfo.bmiHeader.biHeight:(-bmpInfo.bmiHeader.biHeight));
    w = pVideoLogo->width;
    h = pVideoLogo->height;
...
#endif
    /* RGB8888 or RGB1555 */
    pOrigBMPBuf = (FY_U8 *)malloc(h*stride);
    if(NULL == pOrigBMPBuf)
    {
...
int LoadBMPEx(const char *filename, OSD_LOGO_T *pVideoLogo, OSD_COLOR_FMT_E enFmt)
{
    ...
    stride = w*Bpp;
	printf("11Bpp=%d,stride=%d,stride%%4=%d,enFmt=%d\n",Bpp,stride,stride%4,enFmt);
#if 1
	if(stride%4)
	{
        stride = (stride&0xfffc) + 4;
            ...
            if(Bpp == 3)/*.....*/
            {
                switch(enFmt)
                {
                    ...
                    case OSD_COLOR_FMT_RGB4444:
                        ...
                        pDst = (FY_U16*)(pRGBBuf + i*pVideoLogo->stride + j*2);
                        r = *(pStart);
                        g = *(pStart + 1);
                        b = *(pStart + 2);
...
int LoadBMPCanvas(const char *filename, OSD_LOGO_T *pVideoLogo, OSD_COLOR_FMT_E enFmt)
{
    ...
    w = (FY_U16)bmpInfo.bmiHeader.biWidth;
    h = (FY_U16)((bmpInfo.bmiHeader.biHeight>0)?bmpInfo.bmiHeader.biHeight:(-bmpInfo.bmiHeader.biHeight));
