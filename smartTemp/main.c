...
#define LOG_HANDLE
#ifdef  LOG_HANDLE
	#define log_debug(...) zlog_debug(zc,__VA_ARGS__)
...
{ 
    ...
    if(ata_enable_smart(fd) != 0) 
    {
      ...
      */
      ...
      field = ata_search_temperature(values, 0x12);
	  if(!field){
		  printf("ID not accessable\r\n");
		  field = ata_search_temperature(values,0x09);
	  }
	  else printf("time:%d\r\n",*(field+3));
      /* temperature */
...
int log_start_cycle (int device, unsigned char pagenum, unsigned char *pBuf)
{
  struct cdb10hdr *ioctlhdr;
  unsigned char tBuf[1024 + CDB_12_HDR_SIZE];
  unsigned char status;
 
  printf("for start_stop cycle\r\n");
  ...
  ioctlhdr->cdb[5] = 0x00;
  ioctlhdr->cdb[6] = 0x00;
  ioctlhdr->cdb[7] = 0x06+0x06+0x04+0x04;
  ioctlhdr->cdb[8] = 0x00;
  ioctlhdr->cdb[9] = 0x00;
 
  status =  ioctl( device, 1 , &tBuf);
  printf("year:%c%c%c%c\r\n",tBuf[7],tBuf[8],tBuf[9],tBuf[10]) ;
  printf("cycle:%02x%02x%02x%02x\r\n",tBuf[35],tBuf[36],tBuf[37],tBuf[38]) ;
...
int scsi_get_temperature(int fd)
{
    ...
    if (log_start_cycle(fd , 0x0e, buf) != 0)
    {
...
int sata_get_temperature(int fd)
{
  ...
  field = sata_search_temperature(values, 0x04);
  if(field){
	  log_info("Start_Stop_Count[4]:    %d",*(field+3)+(((u16)(*(field+6)) & (u16)0x00ffU) << 8));
  }
  field = sata_search_temperature(values, 0x09);
  if(field){
	  log_info("Power-On Hours[9]:      %d",*(field+3)+(((u16)(*(field+6)) & (u16)0x00ffU) << 8));
  }


  field = sata_search_temperature(values, 0x0c);
  if(field){
	  log_info("Power Cycle Count[12]:  %d",*(field+3)+(((u16)(*(field+6)) & (u16)0x00ffU) << 8));
  }


  field = sata_search_temperature(values, 197);
  if(field){
	  log_info("Current_Pending_Sector[197]:%d",*(field+3));
  }

  field = sata_search_temperature(values, 200);
  if(field){
	  log_info("Multi-Zone Error Rate[200]:%d",*(field+3));
  }
  field = sata_search_temperature(values, 0x05);
  if(field){
	  log_info("Relocated Sector Count[5]:%d",*(field+3));
  }
...
int main(int argc, char* argv[])
{
    ...
    if(value > 0)
        log_info("%s: %s, temperature: %d C", type, mode, value);
    else if(mode)
        log_info("%s: %s:  no sensor", device, mode);
    else
        log_info("get temperature failed!");
    close(fd);
    free(mode);
    return 0;
}
