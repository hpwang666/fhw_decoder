...
void pkgProcess(u_char *rtpPkg)
{
	...
	static SAMPLE_VO_MODE_E layerMode = VO_MODE_4MUX;
	rtpPkg_t pkg = (rtpPkg_t)rtpPkg;
	if(pkg->cmd == 88){//切屏信号
		rtspChnStatus = (rtspChnStatus_t)pkg->data;
		//如果是四分屏，就会接收到4次切屏信号
		if(rtspChnStatus->subwin == layerMode){//窗口不需要重新分割
			if(rtspChnStatus->update){

				if(decEnv->dec25[rtspChnStatus->id].err){
					FY_MPI_VDEC_EnableUserPic((VDEC_CHN)rtspChnStatus->id,FY_FALSE);
			...
			for(i=0;i<layerMode;i++){
				...
				if(decEnv->dec25[i].err){
					FY_MPI_VDEC_EnableUserPic((VDEC_CHN)i,FY_FALSE);
...
#if 0
				s32Ret = HI_MPI_VO_ClearChnBuffer(VO_LAYER_VHD0, i, HI_TRUE);
				if (HI_SUCCESS != s32Ret)
				{
					printf("pause vo chn failed! \n");
			...
			vdec_vo_deinit_layer(layerMode, FY_VO_LAYER_VHD0, 0, layerMode);
			//vout_stop_chn(VO_LAYER_VHD0,layerMode);
			switch (rtspChnStatus->subwin)
			{
				case 1:layerMode=VO_MODE_1MUX;	break;
				case 4:layerMode=VO_MODE_4MUX;	break;
				//case 6:layerMode=VO_MODE_6MUX;	break; 
			...
			//s32Ret = vout_start_chn(VO_LAYER_VHD0, layerMode);
			s32Ret = vdec_vo_init_layer(layerMode, FY_VO_LAYER_VHD0, 0, layerMode);
			if(s32Ret != FY_SUCCESS)
			{		
			...
			/* for(i=0; i<4; i++)
			   {
			   s32Ret = vout_bind_vpss(VO_LAYER_VHD0, i, i, VPSS_CHN0);
			   if(s32Ret != HI_SUCCESS)
			   {	    
	...
	//	if(pkg->cmd < layerMode)//防止通道越界 允许解码
		if(pkg->cmd < 4)//TODO 这里要和 1 4 6 9 一致  防止通道越界 允许解码
			//printf("%d %d\r\n",pkg->cmd,pkg->len);
