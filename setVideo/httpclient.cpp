...
int recv_noblock(httpclient_t ct)
{
	...
	timeout.tv_sec = 4;
	timeout.tv_usec =1000;       /* 连接超时时长：100ms */
					...
					if(FD_ISSET(fds,&read_fds)){
						if(fds == ct->httpFD){
							if (ioctl(ct->httpFD, FIONREAD,&available) == -1) {
								...
								recv_len= recv(ct->httpFD, ct->httpBuf+ct->pkgLen, 8192, 0);	
							}
						}
					}
				}
...
//	 res = recv(ct->httpFD, ct->httpBuf, 8192, 0);//+++++++++++++++++++
	res = recv_noblock(ct);
	if (res <= 0)
	{
		printf("error1\r\n");
	...
	if (strcmp(code_buf, "401") == 0)
	{
		...
		else
		{
			...
			while(ct->contentLen +(tail+4-(u_char *)ct->httpBuf)!=ct->pkgLen){
				
				res = recv_noblock(ct);
				ct->pkgLen +=res;
			}
			snprintf(ct->recvContent,ct->contentLen-1,"%s",tail+4);//以后得换行符不要
			return ct->pkgLen;
		}			
	}
...
int httpClientPut(httpclient_t ct, char *uri, char *content)
{
	...
	if (ct->header)
	{
		memset((ct->header), '\0', 4096);
	}
	if (ct->body)
	{
		memset((ct->body), '\0', 4096);
	}
	...
	if(strlen(ct->authForPtz)>0){
		strcat(ct->header, "Authorization: ");
		strcat(ct->header, ct->authForPtz);
		strcat(ct->header, "\r\n");
	}
	...
	else if (strcmp(code_buf, "200") == 0){
		printf(">>>good no need do auth\r\n");
	}
	else
	{
		printf("##warning:%s,\r\n##RES:%s\r\n",ct->header,ct->httpBuf);
...
{
	int res = 0;
	snprintf(ct->httpBuf, 8192, "%s", ct->header);
...
httpclient_t httpClientCreat(char *ip, char *user, char *passwd)
{
	...
	ct->method=(char *)calloc(1, 32);
	ct->header = (char *)calloc(1, 4096);
	ct->body = (char *)calloc(1, 4096);
	ct->recvContent=(char *)calloc(1, 4096);
	ct->sendContent=(char *)calloc(1, 4096);
	ct->authForPtz=(char *)calloc(1, 4096);
	ct->httpBuf = (char *)calloc(1, 8192);
	return ct;
}
...
int httpClearConn(httpclient_t ct)
{
	...
	if (ct->recvContent)
	{
		memset((ct->recvContent), '\0', 4096);
	}
	if (ct->sendContent)
	{
		memset((ct->sendContent), '\0', 4096);
	}
